...
int main(int argc, char* argv[])
{
    ...
    SDL_Thread* key_thread; //key_threadを用いる
    // ? // keyboard_threadを用いる
    // ***** STEP_3_END *****
    ...
    key_thread = SDL_CreateThread(keyboard_func, "key_thread", mtx); // key_threadを作成し、スレッド関数wii_funcを実行（引数mtx）
    ...
    SDL_WaitThread(key_thread, NULL);// keyboard_threadの処理終了を待つ
    // ***** STEP_5_END *****
